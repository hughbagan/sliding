...
var oval_radius := Vector2(230.0, 15.0) # the ellipse that the actors move along
...
var tween_x := []
var tweening := false
var tween_speed := 1.0 # in seconds
...
func _ready():
	...
	for i in range(actors.size()):
		# Set positions for the opening tween
	...
	$BackgroundTween.interpolate_property($Background, "offset", $Background.offset, Vector2(-480.0, 0.0), tween_speed, Tween.TRANS_QUAD, Tween.EASE_OUT)
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("ui_left"):
		for i in range(actors.size()):
			actors[i].x -= 0.01
			$Background.set_offset(Vector2($Background.get_offset().x + 5, 0))
		...
		$BackgroundTween.interpolate_property($Background, "offset", $Background.offset, Vector2(-1140.0, 0.0), tween_speed, Tween.TRANS_QUAD, Tween.EASE_OUT)
		$BackgroundTween.start()
